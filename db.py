...
try:
    ...
    RAW_TO_KWH = 100  # fallback se config non disponibile
...
class TelemetriaDB:
    ...
    def ottieni_energia_iniziale_commessa(self, commessa_tx):
        """Ottiene l'energia iniziale di una commessa dai record precedenti"""
        self.open()
        # Cerca l'ultimo record con stato 'in_lavorazione' per questa commessa
        self.cur.execute('''
            SELECT energia_iniziale_wh FROM telemetria 
            WHERE commessa_tx = ? AND stato_lavorazione = 'in_lavorazione'
    ...
    def aggiorna_stato_commessa(self, commessa_tx, nuovo_stato):
        """Aggiorna lo stato di lavorazione per una commessa specifica"""
        self.open()
        # Aggiorna l'ultimo record per questa commessa
        self.cur.execute('''
            UPDATE telemetria 
            ...
            ORDER BY timestamp DESC
            LIMIT 1
        ''', (nuovo_stato, commessa_tx))
        self.conn.commit()
        self.close()

    def ottieni_dati_commessa_completi(self, commessa_tx):
        """Ottiene tutti i dati energetici di una commessa completata"""
        self.open()
        self.cur.execute('''
            SELECT energia_iniziale_wh, energia_finale_wh, energia_consumata_wh, stato_lavorazione
            FROM telemetria 
        ...
        if result:
            return {
                'energia_iniziale_wh': result[0],
                'energia_finale_wh': result[1],
                'energia_consumata_wh': result[2],
                'stato_lavorazione': result[3]
            }
        return None

    def aggiorna_record_completo(self, commessa_tx, new_data):
        """Aggiorna il record esistente con i dati energetici completi"""
        self.open()
        
        # Se new_data contiene solo i dati energetici (caso semplificato)
        if 'energia_iniziale_wh' in new_data and 'Commessa RX' not in new_data:
            self.cur.execute('''
                UPDATE telemetria SET
                    energia_iniziale_wh = ?,
                    energia_finale_wh = ?,
                    energia_consumata_wh = ?,
                    stato_lavorazione = 'completata'
                WHERE rowid = (SELECT rowid FROM telemetria WHERE commessa_tx = ? AND (stato_lavorazione = 'in_lavorazione' OR stato_lavorazione = 'non_iniziata') ORDER BY timestamp DESC LIMIT 1)
            ''', (
                new_data.get('energia_iniziale_wh', 0),
                new_data.get('energia_finale_wh', 0),
                new_data.get('energia_consumata_wh', 0),
                commessa_tx
            ))
        else:
            # Caso completo con tutti i dati
            self.cur.execute('''
                UPDATE telemetria SET
                    timestamp = ?,
                    commessa_rx = ?,
            ...
            ''', (
                ...
                new_data.get('Descrizione commessa', ''),
                new_data.get('energia_iniziale_wh', 0),
                new_data.get('energia_finale_wh', 0),
                new_data.get('energia_consumata_wh', 0),
                new_data.get('stato_lavorazione', 'in_lavorazione'),
                commessa_tx
            ))
    ...
    def aggiorna_descrizione_commessa(self, commessa_tx, descrizione):
    ...
    def calcola_energia_commessa(self, commessa_tx):
        ...
        # Trova il record della commessa con i dati energetici già calcolati
        self.cur.execute('''
        ...
        commessa_record = self.cur.fetchone()
        if not commessa_record:
            self.close()
            return None
        
        energia_consumata = commessa_record[0]
        energia_iniziale = commessa_record[1]
        energia_finale = commessa_record[2]
        progressivo_finale = commessa_record[3]
        timestamp = commessa_record[4]
        descrizione = commessa_record[5] or ''
        stato = commessa_record[6]
        ...
        return {
            ...
            'energia_consumata_wh': energia_consumata,
            'energia_consumata_kwh': energia_consumata / RAW_TO_KWH,
            'energia_iniziale_wh': energia_iniziale,
            'energia_finale_wh': energia_finale,
            'progressivo_finale': progressivo_finale,
            'timestamp': timestamp,
    ...
    def calcola_energia_tutte_commesse(self):
        ...
        # Ottieni tutti i record completati con i dati energetici già calcolati
        self.cur.execute('''
        ...
        for record in records:
            commessa_tx = record[0]
            energia_consumata = record[1]
            energia_iniziale = record[2]
            energia_finale = record[3]
            progressivo_finale = record[4]
            timestamp = record[5]
            descrizione = record[6] or ''
            stato = record[7]
            
            commesse_energia.append({
                'commessa_tx': commessa_tx,
                'energia_consumata_wh': energia_consumata,
                'energia_consumata_kwh': energia_consumata / RAW_TO_KWH,
                'energia_iniziale_wh': energia_iniziale,
